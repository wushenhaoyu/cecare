...
var Utils = {
     ...
     * @return 返回终点数组
     */
    getEndLocation(_location) {
        var to = _location.split(";");
     ...
     * 获取location参数
     */
    getLocationParam(_location2) {
        if (typeof _location2 == "string") {
            var locationArr = _location2.split(",");

            if (locationArr.length === 2) {
                _location2 = {
                    latitude: _location2.split(",")[0],
                    longitude: _location2.split(",")[1]
                };
            } else {
                _location2 = {};
            }
     ...
     */
    checkLocation(param) {
        var _location3 = this.getLocationParam(param.location);

        if (!_location3 || !_location3.latitude || !_location3.longitude) {
            var errconf = this.buildErrorConfig(
                ERROR_CONF.PARAM_ERR,
        ...
        if (feature == "search") {
            ...
            for (var i = 0; i < searchResult.length; i++) {
                searchSimplify.push({
                    ...
                    latitude:
                        (searchResult[i].location &&
                            searchResult[i].location.lat) ||
                        null,
                    longitude:
                        (searchResult[i].location &&
                            searchResult[i].location.lng) ||
                        null,
                    address: searchResult[i].address || null,
                    category: searchResult[i].category || null,
                    tel: searchResult[i].tel || null,
                    adcode:
                        (searchResult[i].ad_info &&
                            searchResult[i].ad_info.adcode) ||
                        null,
                    city:
                        (searchResult[i].ad_info &&
                            searchResult[i].ad_info.city) ||
                        null,
                    district:
                        (searchResult[i].ad_info &&
                            searchResult[i].ad_info.district) ||
                        null,
                    province:
                        (searchResult[i].ad_info &&
                            searchResult[i].ad_info.province) ||
                        null
                });
            }
            ...
            for (var i = 0; i < suggestResult.length; i++) {
                suggestSimplify.push({
                    ...
                    latitude:
                        (suggestResult[i].location &&
                            suggestResult[i].location.lat) ||
                        null,
                    longitude:
                        (suggestResult[i].location &&
                            suggestResult[i].location.lng) ||
                        null,
            ...
            if (reverseGeocoderResult.pois) {
                ...
                for (var i = 0; i < pois.length; i++) {
                    poisSimplify.push({
                        ...
                        latitude:
                            (pois[i].location && pois[i].location.lat) || null,
                        longitude:
                            (pois[i].location && pois[i].location.lng) || null,
                        address: pois[i].address || null,
                        category: pois[i].category || null,
                        adcode:
                            (pois[i].ad_info && pois[i].ad_info.adcode) || null,
                        city: (pois[i].ad_info && pois[i].ad_info.city) || null,
                        district:
                            (pois[i].ad_info && pois[i].ad_info.district) ||
                            null,
                        province:
                            (pois[i].ad_info && pois[i].ad_info.province) ||
                            null
                    });
                }
            ...
            var geocoderSimplify = {
                ...
                latitude:
                    (geocoderResult.location && geocoderResult.location.lat) ||
                    null,
                longitude:
                    (geocoderResult.location && geocoderResult.location.lng) ||
                    null,
                adcode:
                    (geocoderResult.ad_info && geocoderResult.ad_info.adcode) ||
                    null,
                province:
            ...
            switch (statusCode) {
                ...
                case ERROR_CONF.WX_OK_CODE: {
                    ...
                    if (data.status === 0) {
                        ...
                        param.complete(
                            that.buildErrorConfig(data.status, data.message)
                        );
                    }
    ...
    locationProcess(param, locationsuccess, locationfail, locationcomplete) {
        ...
        if (!param.location) {
            ...
            var _location4 = Utils.getLocationParam(param.location);

            locationsuccess(_location4);
        }
    }
};
