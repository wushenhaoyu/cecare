...
const apiObj = {startBeaconDiscovery:{fn: function fn (obj = {}) {
      ...
      return my.disconnectBLEDevice(obj)
            ...
            if(that){
                utils.warn(
                    `createVideoContext暂不支持传递实例this`, {
        ...
        return my.removeStorage({
            ...
            function Query() {
                ...
                this.select = function (p) {
                    this.query.select(p);
                    this.query.select(p).node = ()=>{
                        utils.warn(
                           `支付宝暂不支持使用SelectorQuery NodesRef.node`, {
                    ...
                    this.query.boundingClientRect().exec(function (ret) {
                        if (self._selectType) {
                            ...
                            if (Array.isArray(ret) && ret.length === 1) {
                                ret = ret[0];
                            }else if (Array.isArray(ret) && ret.length > 1){
                                utils.warn(
            ...
            return animation;
        },},showShareMenu:{fn: function fn (obj = {}) {
